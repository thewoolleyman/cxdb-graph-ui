...
**IMPORTANT:** This loop takes approximately 10-15 minutes per round. You MUST set the Bash tool timeout to 600000 (the maximum).
...
The bash script handles everything: invoking sub-skills via `claude -p`, checking exit conditions, tracking state, and printing the final report. All stdout and stderr from nested processes streams directly to the caller — no log files, no buffering. Do NOT add any loop logic here — just parse args and run the script.
