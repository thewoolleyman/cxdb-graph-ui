...
now=$(date +%s)
output=$(bash "$CHECK_TIMEOUT" "$now" 10 "test-label" 2>&1) || true
rc=$?
# Should have just started, so well within 10 minutes
# But we need to capture exit code properly
set +e
...
boundary=$(( now - 300 ))  # exactly 5 minutes ago
set +e
...
# Test 4: One second before boundary
echo ""
echo "--- Test: one second before boundary ---"
almost=$(( now - 299 ))  # 4m59s ago
set +e
output=$(bash "$CHECK_TIMEOUT" "$almost" 5 "almost" 2>&1)
rc=$?
set -e
assert_exit "just before boundary returns 0" 0 "$rc" "$output"

# Test 5: Zero timeout (should immediately timeout)
echo ""
echo "--- Test: zero timeout ---"
set +e
output=$(bash "$CHECK_TIMEOUT" "$past" 0 "zero" 2>&1)
rc=$?
set -e
...
set +e
output=$(bash "$CHECK_TIMEOUT" "$now" 9999 "large" 2>&1)
rc=$?
set -e
...
set +e
output=$(bash "$CHECK_TIMEOUT" "$now" 10 2>&1)
rc=$?
set -e
