...
**cxdb-aligned frontend toolchain.** The frontend uses the same build tools as the cxdb repository: Vite + React 18, TypeScript (strict mode), Tailwind CSS v3, and pnpm v9. The build produces static assets served by the Rust server. This ensures consistency, testability, and type safety across the CXDB platform.
...
**Why browser-side DOT rendering.** The `@hpcc-js/wasm-graphviz` npm package compiles Graphviz to WebAssembly and runs entirely in the browser. It is installed via pnpm and bundled by Vite (which handles WASM asset loading). This avoids requiring the `dot` binary on the host, supports interactive SVG manipulation, and renders DOT changes without server restart.
