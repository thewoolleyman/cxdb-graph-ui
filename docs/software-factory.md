...
**What it provides:**
- **Real-time monitoring** — web console at `$KILROY_CXDB_HOST:9120` (via Tailscale) with live SSE updates
...
**Architecture:**
- Binary protocol on `$KILROY_CXDB_HOST:9109` — where Kilroy streams turns during execution
- HTTP API on `$KILROY_CXDB_HOST:9110` — for querying contexts and turns
- Nginx frontend on `$KILROY_CXDB_HOST:9120` — the web console UI

CXDB runs on the central server, managed by `kilroy cxdb`. Access is via Tailscale only (bound to the Tailscale IP).
...
|-------|-------|-------------|
| `/kilroy:setup` | `/kilroy:setup` | One-time setup: builds Kilroy binary from `../kilroy`, verifies CXDB is reachable on the central server via Tailscale, verifies prereqs (go, ruby, claude CLI, API key). |
...
### Build Kilroy and Verify CXDB

```bash
cd ../kilroy && go build -o kilroy ./cmd/kilroy
./script/start-cxdb.sh   # verifies remote CXDB is healthy
```

Verify: `curl -sf http://$KILROY_CXDB_HOST:9110/healthz`

CXDB runs on the central server (`$KILROY_CXDB_HOST`). To start it on the server: `kilroy cxdb start cxdb-graph-ui`
...
The CXDB UI at `http://$KILROY_CXDB_HOST:9120` shows turn-by-turn history. Use `/cxdb:status` for a quick summary.
